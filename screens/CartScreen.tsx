...
const API_URL = 'http://10.168.93.181:3000'; 

interface CartItem {
  plantId: Plant | null; // plantId can be null if deleted
  quantity: number;
  _id: string;
}
...
const CartScreen = () => {
  ...
  const handleCheckout = async () => {
    try {
        ...
        if (totalAmount < 1) {
           Alert.alert('Error', 'Cart is empty or invalid.');
           return;
        }
  ...
  const calculateTotal = () => {
    ...
    return cartItems.reduce((sum, item) => {
        // SAFETY CHECK: Only add if plantId exists
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          // --- FIX STARTS HERE ---
          // If the plant was deleted from the database, plantId will be null.
          // We simply return null to hide this item instead of crashing.
          if (!item.plantId) {
            return null;
          }
          // --- FIX ENDS HERE ---
      ...
      <View style={styles.footer}>
        ...
        <Button title="Proceed to Checkout" onPress={handleCheckout} disabled={calculateTotal() === 0} />
      </View>
    </View>
  );
};
