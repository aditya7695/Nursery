...
const AdminPanelScreen = ({ }: AdminPanelScreenProps) => { // New: Receive the navigation prop
  const [plants, setPlants] = useState<Plant[]>([]);
  const [name, setName] = useState('');
  const [category, setCategory] = useState('');
  const [price, setPrice] = useState('');
  const [description, setDescription] = useState('');
  const [imageUrl, setImageUrl] = useState(''); // <-- 1. ADD NEW STATE
  ...
  const handleAddPlant = async () => {
    ...
    try {
      ...
      // 2. ADD imageUrl to the newPlant object
      const newPlant = { 
        name, 
        category, 
        price: Number(price), 
      ...
      setPrice('');
      setDescription('');
      setImageUrl(''); // <-- 3. CLEAR the imageUrl field
    } catch (error) {
